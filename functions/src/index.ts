/**
 * @fileOverview Firebase Cloud Functions para Simulação de Financiamento - VERSÃO DEFINITIVA 3.3
 * Corrige a extração de dados na Etapa 7 com seletores CSS válidos baseados no HTML real.
 */
...
// CONSTANTES DEFINIDAS NO TOPO DO ARQUIVO (fora da função)
const NAVIGATION_TIMEOUT = 90000; // 90 segundos
const SCREENSHOT_TIMEOUT = 5000; // 5 segundos

// Função auxiliar para capturar screenshots (DEFINIDA ANTES da função principal)
...
(async (request: CallableRequest) => {
    ...
    try {
        ...
        // ETAPA 4 - Enquadramento (VERSÃO DEFINITIVA CORRIGIDA)
        ...
        // CORREÇÃO: Estratégia robusta para encontrar e clicar no enquadramento
        let cliqueSucesso = false;

        // Estratégia 1: Tentar seletor específico com waitForSelector mais tolerante
        try {
            console.log(`[DEBUG] Estratégia 1: Buscando seletor específico...`);
            
            // CORREÇÃO: Usar waitForSelector com estado 'attached' em vez de 'visible'
            ...
            // CORREÇÃO: Clique com Promise.all para capturar navegação
            await Promise.all([
            ...
            try {
                ...
                // CORREÇÃO: Buscar elemento que contenha "3074" no texto
                const elemento3074 = await page.evaluateHandle(() => {
        ...
        // CORREÇÃO: Não tentar selecionar apólice - deixar o onChange automático funcionar
        // O evento onChange no HTML já dispara automaticamente o submit
...
// =======================================================================================
	...
	console.log(`[DEBUG] ✅ Etapa 6 concluída`);
        ...
        // CORREÇÃO: Verificar se takeScreenshot existe antes de chamar
        if (page && typeof takeScreenshot === 'function') {
            await takeScreenshot(page, 'erro_simulacao_final');
        } else {
            console.error(`[DEBUG] Função takeScreenshot não disponível`);
        }
