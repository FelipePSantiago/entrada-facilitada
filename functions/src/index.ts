...
export const verifyTokenAction = onCall({ ...publicOptions, maxInstances: 20 }, withSecurity((request: CallableRequest) => actions.verifyTokenAction({ uid: ensureAuth(request), token: request.data.token }), { requireAuth: true, rateLimitConfig: RATE_LIMIT_CONFIGS.AUTH, allowedOrigins }));
