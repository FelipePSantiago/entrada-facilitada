...
const getStatusBadgeClass = (status: UnitStatus) => {
  switch (status) {
    case 'Disponível':
 return 'unit-card-status-badge-apple unit-status-badge-apple-available';
    case 'Vendido':
      return 'unit-status-badge-apple unit-status-badge-apple-sold';
    case 'Reservado':
      return 'unit-status-badge-apple unit-status-badge-apple-reserved';
    case 'Indisponível':
      return 'unit-status-badge-apple unit-status-badge-apple-unavailable';
    default:
      return 'unit-status-badge-apple';
  }
};
...
const UnitCard = memo(({ unit, isReservaParque, onUnitSelect, style }: UnitCardProps) => {
  ...
  return (
 <div className="unit-card-wrapper-apple" style={style}>
          <Card 
              className="unit-card-apple"
              onClick={handleClick}
          >
 <CardHeader className="unit-card-header-apple">
                  <div>
                      <p className="unit-card-name-apple">{unitDisplay}</p>
                      <p className="unit-card-number-apple">Unidade {unit.unitNumber}</p>
                      <p className="unit-card-floor-apple">{unit.floor}</p>
                  </div>
                  <div className="unit-card-status-badge-apple">
                  {unit.status}
                  </div>
              </CardHeader>
 <CardContent className="unit-card-content-apple flex flex-col gap-2">
                  <div className="unit-card-sale-value-group-apple">
                      <span className="unit-card-sale-label-apple">Venda:</span>
                      <span className="unit-card-sale-value-apple">{centsToBrl(unit.saleValue)}</span>
                  </div>
                  <div className="unit-card-details-grid-apple">
                      <div className="unit-detail-item-apple">
                          <Grid3X3 className="unit-detail-icon-apple" />
 <span className="unit-detail-text-apple text-sm"><strong>Tipologia:</strong> {unit.typology}</span>
                      </div>
                      <div className="unit-detail-item-apple">
 <Ruler className="h-4 w-4" />
 <span className="unit-detail-text-apple text-sm"><strong>Área:</strong> {(unit.privateArea).toFixed(1)}m²</span>
                      </div>
                      <div className="unit-detail-item-apple">
 <Sun className="h-4 w-4" />
 <span className="unit-detail-text-apple text-sm"><strong>Sol:</strong> {unit.sunPosition}</span>
                      </div>
                      <div className="unit-detail-item-apple">
 <Car className="h-4 w-4" />
 <span className="unit-detail-text-apple text-sm"><strong>Vagas:</strong> {unit.parkingSpaces}</span>
                      </div>
                  </div>
                  <div className="unit-card-appraisal-group-apple">
                      <span className="unit-card-appraisal-label-apple">Avaliação:</span>
                      <span className="unit-card-appraisal-value-apple">{centsToBrl(unit.appraisalValue)}</span>
                  </div>
              </CardContent>
          </Card>
      </div>
  ...
  id?: string 
    ...
    return (
        <FormField
            ...
            render={({ field }) => (
                <FormItem>
                    <FormLabel className="form-label-apple">{label}</FormLabel>
                    <FormControl>
                        <div className="relative">
                            <DollarSign className="form-currency-icon-apple text-gray-500" /> {/* Corrected className */}
                            <CurrencyInput
                                value={(field.value as number) * 100}
                                onValueChange={(cents) => field.onChange(cents === null ? 0 : cents / 100)}
                                className="form-currency-input-apple"
                                readOnly={readOnly}
...
export function PaymentFlowCalculator({ properties, isSinalCampaignActive, sinalCampaignLimitPercent, isTutorialOpen, setIsTutorialOpen }: PaymentFlowCalculatorProps) {
  ...
  const handleApplyMinimumCondition = useCallback(() => {
    ...
    if (!validation.isValid) {
      ...
      toast({
        ...
        description: "Os pagamentos foram ajustados. Calculando resultados...S",
      });
    }
  ...
  return (
    <div className="payment-flow-calculator-container-apple" onPaste={handlePaste}>
      <Card className="payment-flow-calculator-card-apple">
        <CardHeader className="payment-flow-calculator-header-apple">
          <div className="payment-flow-calculator-title-container-apple">
            <CardTitle className="payment-flow-calculator-title-apple text-2xl font-semibold">
              <Calculator className="payment-flow-calculator-icon-apple h-7 w-7" />
              Simulador de Fluxo de Pagamento
            </CardTitle>
            <CardDescription className="text-gray-600 dark:text-gray-400 text-sm mb-6">
              Preencha os dados abaixo para simular as condições de pagamento do imóvel.
            </CardDescription>
          </div>
          
          <div className="automated-simulation-toggle-container-apple">
            <Switch
              id="automated-simulation"
              checked={isAutomatedSimulationEnabled}
              onCheckedChange={setIsAutomatedSimulationEnabled}
            ></Switch>
            <Label htmlFor="automated-simulation" className="text-sm text-gray-700 dark:text-gray-300">
              Simulação Automatizada
            </Label>
          </div>
        </CardHeader>
        <CardContent className="payment-flow-calculator-content-apple">
          <Form {...form}>
            <form onSubmit={form.handleSubmit(onSubmit)} className="payment-flow-calculator-form-apple">
              {isAutomatedSimulationEnabled && (
                <Card className="automated-simulation-card-apple">
                  <CardHeader className="automated-simulation-header-apple">
                    <CardTitle className="automated-simulation-title-apple">
                      <ShieldCheck className="automated-simulation-icon-apple" />
                      Dados do Cliente
                    </CardTitle>
                  ...
                  <CardContent className="automated-simulation-content-apple flex flex-col gap-4">
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 mb-4">
 <div className="automated-simulation-field-group-apple flex flex-col gap-2 form-item">
 <Label htmlFor="caixa-renda" className="automated-simulation-label-apple text-xs font-medium text-gray-500 dark:text-gray-400">Renda Bruta Mensal</Label>
                        <div className="relative form-currency-input-container-apple">
                          <DollarSign className="form-currency-icon-apple text-gray-500" />
                          <Input 
                            id="caixa-renda" 
                            name="renda" 
                            type="text"
                            ...
                            placeholder="R$ 0,00"
                            className="automated-simulation-input-apple"
                            required 
                          />
                        </div>
                      </div>
 ...
 <div className="automated-simulation-field-group-apple flex flex-col gap-2 form-item">
 <Label htmlFor="caixa-data-nascimento" className="automated-simulation-label-apple text-xs font-medium text-gray-500 dark:text-gray-400">Data de Nascimento</Label>
                        <Input
 ...
 <div className="automated-simulation-field-group-apple flex flex-col gap-2 form-item">
 <Label htmlFor="caixa-sistema-amortizacao" className="automated-simulation-label-apple text-xs font-medium text-gray-500 dark:text-gray-400">Sistema de Amortização</Label>
                      <Select
                        value={customerData.sistemaAmortizacao} 
 onValueChange={(value) => setCustomerData(prev => ({ ...prev, sistemaAmortizacao: value }))}
                      >
 <SelectTrigger className="automated-simulation-select-trigger-apple h-9">
                    ...
                    <Button 
                      ...
                      className="automated-simulation-button-apple"
                    >
                    ...
                    {caixaSimulationResult && (
                      <Card className="caixa-results-card-apple">
                        <CardHeader className="caixa-results-header-apple">
 <CardTitle className="caixa-results-title-apple text-lg font-semibold">Resultados da Simulação Caixa</CardTitle>
                        </CardHeader>
                        <CardContent className="caixa-results-content-apple">
                          <div className="caixa-results-grid-apple">
                            <div className="caixa-result-item-apple flex justify-between gap-x-2">
 <p className="caixa-result-label-apple text-sm text-gray-600 dark:text-gray-400">Prazo:</p>
 <p className="caixa-result-value-apple text-sm font-medium text-gray-800 dark:text-gray-200">{caixaSimulationResult.Prazo || 'N/A'}</p>
                            </div>
                            <div className="caixa-result-item-apple flex justify-between gap-x-2">
 <p className="caixa-result-label-apple text-sm text-gray-600 dark:text-gray-400">Valor Total Financiado:</p>
 <p className="caixa-result-value-apple caixa-result-value-green-apple text-sm font-medium text-gray-800 dark:text-gray-200">
                                {corrigirFormatoValor(caixaSimulationResult.Valor_Total_Financiado || 'N/A')}
                              </p>
                            </div>
                            <div className="caixa-result-item-apple flex justify-between gap-x-2">
 <p className="caixa-result-label-apple text-sm text-gray-600 dark:text-gray-400">Primeira Prestação:</p>
 <p className="caixa-result-value-apple caixa-result-value-blue-apple text-sm font-medium text-gray-800 dark:text-gray-200">
                                {corrigirFormatoValor(caixaSimulationResult.Primeira_Prestacao || 'N/A')}
                              </p>
                            </div>
                            <div className="caixa-result-item-apple flex justify-between gap-x-2">
                              <p className="caixa-result-label-apple text-sm text-gray-600 dark:text-gray-400">Juros Efetivos:</p>
                              <p className="caixa-result-value-apple caixa-result-value-purple-apple text-sm font-medium text-gray-800 dark:text-gray-200">
              ...
              <div className="form-grid-2-cols-apple">
              <FormField
              control={form.control}
              name="propertyId"
              render={({ field }) => (
                <FormItem>
                  <FormLabel className="form-label-apple">Empreendimento</FormLabel>
                  <Select 
                    value={field.value || ""}
                    onValueChange={(value) => {
                  ...
                  >
                    <FormControl>
                      <SelectTrigger className="form-select-trigger-apple">
            ...
            />
                <FormField
                  ...
                  render={({ field }) => (
                    <FormItem>
                      <FormLabel className="form-label-apple">Unidade Selecionada</FormLabel>
                      <div className="flex gap-2">
                        <FormControl>
                          <Input
                            ...
                            readOnly
                            className={cn(
                              "form-input-apple unit-selection-input-apple",
                              isSaleValueLocked 
 ? "bg-gray-100 dark:bg-gray-800 border-gray-300 dark:border-gray-700 text-gray-900 dark:text-gray-100 font-medium" 
 : "bg-gray-50 dark:bg-gray-800 border-gray-200 dark:border-gray-700 text-gray-900 dark:text-gray-100"
                        ...
                        <Button
                          ...
                          className="unit-selection-button-apple"
                        >
                        ...
                        {isSaleValueLocked && (
                          <Button
                            ...
                            className="unit-clear-button-apple"
                          >
              ...
              <div className="flex flex-col gap-4">
                <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-2 sm:gap-4">
                  <h3 className="section-title-apple">Pagamentos</h3>
                  <Button
                    type="button"
                    onClick={() => {
                    ...
                    className="add-payment-button-apple"
                  >
                ...
                <div className="payment-fields-list-apple">
                  {fields.map((field, index) => (
                    <div key={field.id} className="payment-field-item-apple">
                      <FormField
                        control={form.control}
                        name={`payments.${index}.type`}
                        render={({ field }) => (
                          <FormItem className="flex-1 w-full">
                            <FormLabel className="form-label-apple">Tipo</FormLabel>
                      ...
                      <FormField
                        ...
                        render={({ field }) => (
                          <FormItem className="flex-1 w-full">
                            <FormLabel className="form-label-apple">Valor</FormLabel>
                            <FormControl>
                              <CurrencyInput
                      ...
                      <FormField
                        ...
                        render={({ field }) => (
                          <FormItem className="flex-1 w-full">
                            <FormLabel className="form-label-apple">Data</FormLabel>
                            <FormControl>
                              <DatePicker
                      ...
                      <Button
                        ...
                        className="remove-payment-button-apple"
 ...
 <div className="form-grid-3-cols-apple">
                <FormField
                  control={form.control}
                  name="conditionType"
                  render={({ field }) => (
                    <FormItem className="form-item-apple">
                      <FormLabel className="form-label-apple">Condição</FormLabel>
                ...
                <FormField
                  ...
                  render={({ field }) => (
                    <FormItem className="form-item-apple">
                      <FormLabel className="form-label-apple">Número de Parcelas</FormLabel>
                      <FormControl>
                        <Input
                          type="number"
                ...
                <FormField
                  ...
                  render={({ field }) => (
                    <FormItem className="form-item-apple">
                      <FormLabel className="form-label-apple">Participantes no Financiamento</FormLabel>
 ...
 <CurrencyFormField
                ...
                <FormField
                  ...
                  render={({ field }) => (
                    <FormItem className="form-item-apple">
                      <FormLabel className="form-label-apple">Método de Pagamento Cartório</FormLabel>
                      <Select onValueChange={field.onChange} defaultValue={field.value}>
                        <FormControl>
                          <SelectTrigger className="form-select-trigger-apple">
                ...
                <FormField
                  ...
                  render={({ field }) => (
                    <FormItem className="form-item-apple">
                      <FormLabel className="form-label-apple">Parcelas Cartório</FormLabel>
                      <FormControl>
                        <Input
                          type="number"
                          value={field.value || ''} // Garante que o input mostre vazio se o valor for undefined
              ...
              <div className="action-buttons-group-apple">
                <Button type="submit" className="button-primary-apple">
                  <Calculator className="h-4 w-4 mr-2 text-white dark:text-gray-900" />
                  Calcular
                </Button>
                ...
                <Button
                  ...
                  className="button-secondary-apple"
                >
                ...
                <Button
                  ...
                  className="button-destructive-apple"
                >
      ...
      {results && (
        <Card ref={resultsRef} className="results-card-apple">
          <CardHeader className="results-header-apple">
            <CardTitle className="results-title-apple">
              <TrendingUp className="results-icon-apple" />
              Resultados da Simulação
            </CardTitle>
            <CardDescription className="results-description-apple">
              Confira abaixo os detalhes da simulação realizada.
            </CardDescription>
          </CardHeader>
          <CardContent className="results-content-apple">
            <div className="results-metrics-grid-apple">
              <Card className="metric-card-apple">
                <CardContent className="metric-card-content-apple">
                  <div className="metric-label-group-apple">
                    <Wallet className="metric-icon-apple text-blue-600" />
                    <span className="metric-label-apple">Pró-Soluto</span>
                  </div>
                  <p className="metric-value-apple metric-value-blue-apple">
              ...
              <Card className="metric-card-apple">
                <CardContent className="metric-card-content-apple">
                  <div className="metric-label-group-apple">
                    <CreditCard className="metric-icon-apple text-green-600" />
                    <span className="metric-label-apple">Parcela Pró-Soluto</span>
                  </div>
                  <p className="metric-value-apple metric-value-green-apple">
              ...
              <Card className="metric-card-apple">
                <CardContent className="metric-card-content-apple">
                  <div className="metric-label-group-apple">
                    <TrendingUp className="metric-icon-apple text-purple-600" />
                    <span className="metric-label-apple">Taxa de Juros Pró-Soluto</span>
                  </div>
                  <p className="metric-value-apple metric-value-purple-apple">
              ...
              <Card className="metric-card-apple">
                <CardContent className="metric-card-content-apple">
                  <div className="metric-label-group-apple">
                    <ShieldCheck className="metric-icon-apple text-orange-600" />
                    <span className="metric-label-apple">Seguro de Obras (Pago)</span>
                  </div>
                  <p className="metric-value-apple metric-value-orange-apple">
            ...
            <div className="form-grid-2-cols-apple">
                <Card className="cost-summary-card-apple">
                  <CardHeader className="cost-summary-header-apple">
                    <CardTitle className="cost-summary-title-apple">Resumo de Custos</CardTitle>
                  </CardHeader>
                  <CardContent className="cost-summary-content-apple flex flex-col gap-2">
                    <div className="flex flex-col gap-2">
                      <div className="cost-item-apple flex justify-between text-sm"><span>Entrada</span><span className="font-medium">{centsToBrl((results.totalEntryCost || 0) * 100)}</span></div>
                      <div className="cost-item-apple flex justify-between text-sm"><span>Pró-Soluto</span><span className="font-medium">{centsToBrl((results.totalProSolutoCost || 0) * 100)}</span></div>
                      <div className="cost-item-apple flex justify-between text-sm"><span>Financiamento</span><span className="font-medium">{centsToBrl((results.totalFinancedCost || 0) * 100)}</span></div>
                      <div className="cost-item-apple flex justify-between text-sm"><span>Taxas Cartorárias</span><span className="font-medium">{centsToBrl((results.totalNotaryCost || 0) * 100)}</span></div>
                      <div className="cost-item-apple flex justify-between text-sm"><span>Seguro Obra</span><span className="font-medium">{centsToBrl((results.totalInsuranceCost || 0) * 100)}</span></div>
                      <Separator />
                      <div className="flex justify-between font-bold text-sm"><span>Total</span><span className="cost-total-apple">{centsToBrl((results.totalCost || 0) * 100)}</span></div>
                    </div>
                  </CardContent>
                </Card>
                <Card className="income-analysis-card-apple">
                  <CardHeader className="income-analysis-header-apple">
                    <CardTitle className="income-analysis-title-apple">Análise de Renda</CardTitle>
                  </CardHeader>
                  <CardContent className="income-analysis-content-apple flex flex-col gap-4">
                    <div className="flex flex-col gap-4">
                      <div className="income-analysis-item-apple">
                        <div className="flex justify-between mb-2 text-sm"><span className="income-percentage-label-apple">Comprometimento de Renda</span><span className="income-percentage-value-apple">{(results.incomeCommitmentPercentage || 0).toFixed(2)}%</span></div>
                        <div className="progress-bar-container-apple"><div className={`progress-bar-fill-apple ${results.incomeCommitmentPercentage > 50 ? 'bg-red-500' : results.incomeCommitmentPercentage > 30 ? 'bg-yellow-500' : 'bg-green-500'}`} style={{ width: `${Math.min(results.incomeCommitmentPercentage || 0, 100)}%` }} /></div>
                      </div>
                      <div className="income-analysis-item-apple">
                        <div className="flex justify-between mb-2 text-sm"><span className="income-percentage-label-apple">Percentual Pró-Soluto</span><span className="income-percentage-value-apple">{(results.proSolutoCommitmentPercentage || 0).toFixed(2)}%</span></div>
                        <div className="progress-bar-container-apple"><div className={`progress-bar-fill-apple ${results.proSolutoCommitmentPercentage > 100 ? 'bg-red-500' : results.proSolutoCommitmentPercentage > 50 ? 'bg-red-500' : results.proSolutoCommitmentPercentage > 50 ? 'bg-yellow-500' : 'bg-green-500'}`} style={{ width: `${Math.min(results.proSolutoCommitmentPercentage || 0, 100)}%` }} /></div>
                      </div>
              ...
              <div className="flex flex-col gap-4" id="cronograma-pagamentos-section">
                <h3 className="section-title-apple">Cronograma de Pagamentos</h3>
              ...
              <div className="flex flex-col gap-4" id="detalhamento-seguro-obras-section">
                <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-2">
                  <h3 className="section-title-apple">Detalhamento do Seguro de Obras</h3>
                  <Button 
                    type="button"
                    onClick={() => setShowInsuranceDetails(!showInsuranceDetails)}
                    className="toggle-insurance-details-button-apple"
                  >
                    {showInsuranceDetails ? (
                      ...
                      <>
                        <ChevronDown className="toggle-insurance-icon-apple" />
                ...
                {showInsuranceDetails && results?.monthlyInsuranceBreakdown && (
                  <Card className="insurance-details-card-apple">
                    <CardContent className="insurance-details-content-apple">
                      <div className="overflow-x-auto -mx-2 px-2 sm:mx-0 sm:px-0 scrollbar-thin scrollbar-thumb-gray-300">
                        <Table className="insurance-table-apple">
                          <TableHeader className="insurance-table-head-apple">
                            <TableRow className="insurance-table-row-apple">
                              <TableHead className="insurance-table-header-cell-apple">Mês</TableHead>
                              <TableHead className="insurance-table-header-cell-apple">Valor</TableHead>
                              <TableHead className="insurance-table-header-cell-apple">Progresso</TableHead>
                            </TableRow>
                          </TableHeader>
                          <TableBody>
                            {filteredInsuranceBreakdown.map((item, index) => (
                              <TableRow key={index} className="insurance-table-row-apple insurance-details-table-row-apple">
                                <TableCell className="insurance-table-cell-apple">
{item.month}
</TableCell>
                                <TableCell className="insurance-table-cell-apple">
{centsToBrl(item.value * 100)}
</TableCell>
                                <TableCell className="insurance-table-cell-apple">{(item.progressRate * 100).toFixed(1)}%</TableCell>
                              </TableRow>
                            ))}
                          </TableBody>
                        </Table>
              ...
              <div className="flex flex-col sm:flex-row gap-2 sm:gap-4">
                <Button onClick={handleGeneratePdf} disabled={isGeneratingPdf} className="button-primary-apple generate-pdf-button-apple">
      ...
      <Dialog open={isUnitSelectorOpen} onOpenChange={setIsUnitSelectorOpen}>
        <DialogContent className="unit-selector-dialog-content-apple">
          <DialogHeader>
            <DialogTitle className="unit-selector-dialog-title-apple">
              Selecione uma Unidade do Empreendimento {selectedProperty?.enterpriseName || ''}
            </DialogTitle>
            <DialogDescription className="unit-selector-dialog-description-apple">
          ...
          <div className="flex flex-col gap-4">
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
              <div className='flex flex-col gap-2'>
                <Label className="unit-filter-label-apple">Status</Label>
                <Select value={statusFilter} onValueChange={(value: UnitStatus | "Todos") => setStatusFilter(value)}>
                  <SelectTrigger className="unit-filter-select-trigger-apple">
                    <SelectValue />
                  </SelectTrigger>
                  <SelectContent>
              ...
              <div className='flex flex-col gap-2'>
                <Label className="unit-filter-label-apple">Andar</Label>
              ...
              <div className='flex flex-col gap-2'>
                <Label className="unit-filter-label-apple">Tipologia</Label>
              ...
              <div className='flex flex-col gap-2'>
                <Label className="unit-filter-label-apple">Posição Solar</Label>
